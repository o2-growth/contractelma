-- ============================================================================
-- Seed do template "CFO as a Service (Modelo 4 - Oficial, revisado março)" para super_admins.
--
-- ESTRITAMENTE ADITIVA: apenas INSERT WHERE NOT EXISTS.
-- Não altera, deleta ou sobrescreve dados existentes. Idempotente.
--
-- DOCX oficial em: storage/documents/templates/base/cfo-as-a-service-modelo4.docx
...
SELECT
  ur.user_id,
  'CFO as a Service (Modelo 4 - Oficial, revisado março)' AS name,
  'Modelo 4 oficial O2 Inc - Setup + Assessoria de Gestão Financeira Recorrente no modelo CFO AS A SERVICE' AS description,
  $TEMPLATE$# CONTRATO DE PRESTAÇÃO DE SERVIÇOS — CFO AS A SERVICE
...
WHERE ur.role = 'super_admin'
  AND NOT EXISTS (
    ...
    WHERE t.user_id = ur.user_id
      AND t.name = 'CFO as a Service (Modelo 4 - Oficial, revisado março)'
  );
